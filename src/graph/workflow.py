...
def node_scheduler(state: GraphState) -> Dict[str, Any]:
    ...
    # 合并所有历史输入（包括追问补充）
    combined_input = memory.get_combined_input()
    if message and message not in combined_input:
        if combined_input:
            combined_input += "；" + message
        else:
            combined_input = message

    print(f"   📝 合并后输入: {combined_input[:80]}...")

    # 检查是否已达最大追问次数 -> 强制生成
    if memory.clarification_rounds >= memory.max_clarification_rounds:
        print(f"   ⏹️  已达最大追问次数({memory.max_clarification_rounds})，强制生成")
        # 正常调度，不再追问
        schedule_result = scheduler.schedule(combined_input, has_image)
        experts = schedule_result.get("experts", ["style", "mood"])
        return {
            "image_path": image_path,
            "has_image": has_image,
    ...
    # 如果用户补充了信息（有追问历史），检查是否足够生成
    if memory.clarification_rounds > 0:
        missing = scheduler.analyze_missing_info(combined_input)
        # 缺失信息少于2项，且输入有一定长度，认为足够
        if len(missing) < 2 and len(combined_input) >= 8:
            is_ambiguous = False
            print(f"   ✅ 补充后信息足够({len(missing)}项缺失)，直接生成")

    if is_ambiguous:
        remaining = memory.get_remaining_rounds()
        print(f"   ❓ 意图模糊，需要追问（第 {memory.clarification_rounds + 1}/{memory.max_clarification_rounds} 轮，剩余{remaining}次）")
        ...
        print(f"   追问: {questions[0] if questions else '能再详细描述一下吗？'}")
        ...
        return {
            ...
            "questions": questions,
            "scheduled_experts": [],
            "clarification_round": memory.clarification_rounds,
            "max_clarification_rounds": memory.max_clarification_rounds
        }
